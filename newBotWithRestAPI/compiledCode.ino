...
#define ENCODER_A 14  // Encoder Channel A to GPIO 14
#define ENCODER_B 27  // Encoder Channel B to GPIO 27
#define COUNTS_PER_REV 13  // Counting only one edge per channel
#define GEAR_RATIO 600.0  // Gear ratio 1:600
#define WHEEL_DIAMETER 11.0  // Diameter in cm
#define WHEEL_CIRCUMFERENCE (3.1416 * WHEEL_DIAMETER)  // Circumference in cm
...
void sendSensorData(WiFiClient client) {
  ...
  jsonDoc["MPU_X"] = kalAngleX;
  jsonDoc["MPU_Y"] = kalAngleY;
  jsonDoc["Speed_in_RPM"] =  rpm1;
  jsonDoc["Distance_in_m"] = totalDistance;
...
void updateMPU6050() {
    if (millis() - lastMPUCheck >= 1000) {
        ...
        if (Wire.available() == 14) {
            ...
            int16_t tempRaw = Wire.read() << 8 | Wire.read();  // Read temperature data
        ...
        kalAngleX = kalmanX.getAngle(angleAccX, gyroXrate, dt);
        kalAngleY = kalmanY.getAngle(angleAccY, gyroYrate, dt);
        
        Serial.print("Kalman Angle X: "); Serial.print(kalAngleX, 2);
        Serial.print(" | Kalman Angle Y: "); Serial.println(kalAngleY, 2);
    }
}
...
void calculateRPM() {
    ...
    if (currentTime - lastTime >= 1000) {
        ...
        float motorRevolutions = pulseCount / (float)COUNTS_PER_REV;
        float outputRevolutions = motorRevolutions / GEAR_RATIO;
        rpm1 = (motorRevolutions * 60.0) / GEAR_RATIO;
        totalDistance += outputRevolutions * WHEEL_CIRCUMFERENCE;
        Serial.print("RPM: "); Serial.print(rpm1);
        Serial.print(" | Distance Traveled: "); Serial.print(totalDistance / 100.0); Serial.println(" m");
        pulseCount = 0;
